...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['filtrar'])) {
    $motoristaSelecionado = $_GET['motorista'];
   ...
   $viagens =  $viagemController->index($motoristaSelecionado, $dataInicio, $dataFim);
    ...
    $placas = $viagemController->listDistinctPlacas();
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
   ...
   <div style="display: flex; justify-content: space-between; align-items: center;">
         <h1>Relatório Geral de Viagens</h1>
         <button onclick="window.print()" style=" padding: 8px 12px; background-color: #4285f4; color: white; border-radius: 4px; text-decoration: none; cursor:pointer;">Imprimir</button>
    </div>
    <a href="/">Voltar</a>

    <form action="relatorio_geral.php" method="GET">
       <label for="motorista">Motorista:</label>
          <select name="motorista">
                <option value="">Todos</option>
                <?php foreach ($motoristas as $motorista): ?>
                    <option value="<?php echo htmlspecialchars($motorista); ?>" <?php if($motorista == $motoristaSelecionado) echo 'selected'; ?>><?php echo htmlspecialchars($motorista); ?></option>
                <?php endforeach; ?>
            </select><br>
    ...
    <table>
        ...
        <tbody>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="10">Nenhuma viagem registrada.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
